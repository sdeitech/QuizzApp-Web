...
const API_URI_2 = "http://192.168.29.212:9002";
// socket config
let socket_2;
...
console.log(socket_2, "Socket Value from start Question");
// if (socket_2) {
socket_2.on("connect", () => {
    console.log("Connection for Socket from start Question");
});
// }

// get socket data if moderator click on start any question
socket_2.on("startQuestion", async (data) => {
    console.log("Game Action====> ", JSON.stringify(data));
    ...
    // FOR ASSIGNED USER
    store.dispatch({
        type: actionTypes.GAME_ASSIGNED_USER,
        payload: data.userId,
    });
});
...
socket_2.on("user-connected-game", ({ userId, username }) => {
    ...
    if (isModerator) {
        ...
        store.dispatch({
            type: actionTypes.GAME_CLIENT_USERS_REQUEST_ADD,
            payload,
        });
    }
});
...
socket_2.on("joinResult", async ({ roomID, userId, status }) => {
    ...
    if (userId === selfUserId) {
        if (!status) {
            ...
            store.dispatch({
                type: actionTypes.GAME_CLIENT_USERS_REQUEST_ACCEPT_STATUS,
                payload: status,
            });
        }
    }
...
socket_2.on("confirm-disconnect-room", (status) => {
    store.dispatch({
        type: actionTypes.GAME_MODERATOR_LEAVE_ROOM,
        payload: status,
    });
    // Actions.reset("Tabs");
});
...
socket_2.on("nextQuestionModerator", () => {
    ...
    store.dispatch({
        type: actionTypes.GAME_MODERATOR_NEXT_ROOM_COMP,
        payload: true,
    });
});
...
export const _startRoundModerator = ({ roomID }) =>
    
async (dispatch) => {
    try {
        const { questionIndex, roundIndex, clientUsers, currentAssignedUser } =
            store.getState().gameReducer;

        let newUserIdIndex = clientUsers.findIndex(
            (x) => x === currentAssignedUser
        );

        console.log(
            "user id for assign new question => index => old => ",
            newUserIdIndex
        );
        console.log(
            "user id for assign new question => id => old => ",
            currentAssignedUser
        );
        ...
        console.log(
            "user id for assign new question => index => ",
            newUserIdIndex
        );
...
export const _resultSendSocket =
    ({ roomID, resultStatus }) =>
    (dispatch) => {
        try {
            const { questionIndex, roundIndex, clientUsers } =
                store.getState().gameReducer;
...
export const _moderatorAcceptRequest =
    ({ roomID, userId, status, isClosed }) =>
    (dispatch) => {
        try {
            console.log("join-room :: ", { roomID, userId, status, isClosed });
            if (!isClosed)
                socket_2.emit("join-room-response", { roomID, userId, status });
            ...
            dispatch({
                type: actionTypes.GAME_CLIENT_USERS_REQUEST_REMOVE,
                payload,
            });
...
export const _moderatorLeaveRoom =
    ({ roomID }) =>
    (dispatch) => {
        try {
...
// moderator move next
export const _moderatorMoveNext =
    ({ roomID }) =>
    (dispatch) => {
        try {
