...
const Room = React.memo((props) => {
    ...
    const moderatorResponse = (socketId, roomId, status) => {
        try {
            ...
            console.log("response222222222 send is ", status);
        } catch (error) {
            console.log("moderatorResponse22222222222 error => ", error);
        }
    };
    ...
    const onNewChatMessageReceived = (messagePayload) => {
        if (messagePayload.type === "JoinRequest") {
            ...
            const { sender } = messagePayload;

            // console.log(
            //     senderUserId,
            //     sender.roomId,
            //     to,
            //     "smdasdmaskdmaksdmaksdmaksdmaksdmaskd"
            // );
            //
            moderatorResponse(props.socket.id, sender.roomId, true);
