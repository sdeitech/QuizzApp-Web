...
const socketReducers = (state = initialData, action) => {
    switch (action.type) {
        case "JOIN_ROOM_REQ_SEND":
            return { ...state, joinRoomReq: action.data };

        case "IS_MODERATOR":
            return { ...state, isModerator: action.data };

        case "SET_ROOMID":
            return { ...state, roomId: action.data };

        case "ROOM_CREATOR_ID":
            return { ...state, roomCreatorId: action.data };

        case "WAIT_SCREEN":
            return { ...state, waitScreen: action.data };

        case "CLEAR_OTHER_USER_STREAM":
            return { ...state, otherUserSteams: [] };

        case "SOCKET":
            return { ...state, socket: action.data };

        case "UPDATE_OTHER_USER_STREAM":
            return { ...state, otherUserSteams: action.data };

        case "REQUEST_SENDER":
            let rs = [...state.requestSender];
            rs = _.reject(rs, function (item) {
                return item.userdata._id == action.data.userdata._id;
            });
        ...
        case "SET_OTHER_USER_STREAM":
            ...
            let findInprevarr = prevMember.find(
                (item) => item.joinedUserId == userData.joinedUserId
            );
            let memberArr;
            if (findInprevarr) {
